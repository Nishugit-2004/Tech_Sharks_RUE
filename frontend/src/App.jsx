...
function App() {
  ...
  useEffect(() => {
    const savedHistory = localStorage.getItem('curominds_history');
    if (savedHistory) {
  ...
  const saveToHistory = (topic) => {
    if (!topic || history.includes(topic)) return;
    const newHistory = [topic, ...history].slice(0, 10); // Keep last 10
    setHistory(newHistory);
    localStorage.setItem('curominds_history', JSON.stringify(newHistory));
  };

  const onSelectHistory = (topic) => {
    setQuery(topic);
    // Trigger search
    setTimeout(() => {
  ...
  const handleAsk = async (e) => {
    ...
    try {
      const res = await axios.post(`${API_URL}/ask`, { query, level: selectedLevel });
      setSessionData({
  ...
  return (
    <div className="min-h-screen selection:bg-primary-500/30 font-inter">
      ...
      {/* History Panel - Fixed in top-left corner */}
      <HistoryPanel history={history} onSelect={onSelectHistory} />

      <div className="container mx-auto px-4 sm:px-6 sm:pl-72 max-w-5xl pt-10 pb-32 min-h-screen flex flex-col items-center">
        ...
        >
          ...
          {hasStarted && (
             ...
             >
                ...
                <LevelSelector selectedLevel={selectedLevel} onChange={setSelectedLevel} disabled={loading} />
          ...
          <p className={`text-lg sm:text-xl text-gray-400 font-medium tracking-wide mt-2 ${hasStarted ? 'hidden' : 'block'}`}>
            Dive deeper into any topic with our powerful AI engine.
          </p>

          <form id="search-form" onSubmit={handleAsk} className="w-full max-w-3xl mt-12 relative group">
