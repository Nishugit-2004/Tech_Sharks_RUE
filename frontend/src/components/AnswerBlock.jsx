      ...
      {parts.map((part, idx) => {
        if (part.isConcept) {
          ...
          return (
            <span
              key={idx}
              className={`highlight-term flex-inline items-center space-x-1 cursor-pointer px-1.5 py-0.5 rounded-md transition-all ${
                isActive 
                  ? 'bg-primary-500/30 text-primary-200 border-b-2 border-primary-500 font-semibold' 
                  : isSelected
                    ? 'ring-2 ring-primary-500/50 bg-primary-500/20 text-primary-200'
                    : magicSelectMode 
                      ? 'opacity-50 grayscale-[0.5] hover:opacity-100 hover:grayscale-0'
                      : 'bg-teal-500/10 text-teal-200 hover:bg-teal-500/20 border border-teal-500/20'
              }`}
              onClick={(e) => {
                e.stopPropagation();
                if (!magicSelectMode && onTermToggle) onTermToggle(part.text);
              }}
              title={magicSelectMode ? 'Magic Selection Active' : 'Click to explore this concept'}
            >
